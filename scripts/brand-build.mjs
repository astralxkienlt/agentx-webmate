...
async function writeRuntimeConfig(outDir, config, written) {
  ...
  const backendBaseUrl = normalizedServiceUrl(
    services.backendBaseUrl,
    'backendBaseUrl',
  );
  ...
  // Which of the two minted tokens the extension presents to the backend. The
  // wrong one is a guaranteed 401: a backend in userinfo mode hands the bearer
  // to the issuer, which recognises only the opaque access token it minted,
  // while a backend that checks signatures locally needs the ID token. Omitted,
  // the ID token is sent — what every build before this key existed did.
  const oidcBearerToken = String(services.oidcBearerToken || 'id_token').trim();
  if (!['id_token', 'access_token'].includes(oidcBearerToken)) {
    throw new Error(
      "brand.config.json services.oidcBearerToken must be 'id_token' or 'access_token'",
    );
  }
  // Defaults to true: a product that copies this config must not lose replay
  // protection by omission. Only an explicit `false` turns it off, and the
  // reason belongs in brand.config.json next to it.
  const oidcSendNonce = services.oidcSendNonce !== false;
