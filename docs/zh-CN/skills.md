...
将其加载到运行中。可在设置 → 技能中管理：导入技能文本、`.md` 文件（文件选择器或
拖放）或 URL；就地编辑任何已启用的技能；或移除任何内置技能。表单会预览草稿将
保存的名称、摘要、模式、意图和工具，因此格式错误的 `webbrain-skill` 代码块在
保存前即可发现。

编辑会保留技能的 id 和位置。编辑打包技能（或修改 URL 导入技能的内容）会把它
转换为同 id 的自定义文本：不再跟随打包/URL 版本，删除编辑副本也不会重新播种
打包默认技能。
