...
(function () {
  ...
  function fail() {
    ...
    try {
      ...
      console.error('[netMind] Panel module never signalled boot; offering manual reload.', firstError || '(no captured error)');
    } catch (renderError) {
      // The console line below is the last remaining diagnostic surface.
      console.error('[netMind] Boot watchdog could not render its notice:', renderError);
    }
  }
