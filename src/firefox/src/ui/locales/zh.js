...
export default {
  ...
  'sp.permmode.auto': '自动',
  'sp.permmode.auto.desc': "WebBrain 自行跳转、点击和输入；在提交表单、运行页面脚本、下载、上传、写入 API 或安排任务前仍会询问。",
  ...
  'sp.model.open': '选择模型',
  'sp.model.heading': '模型',
  'sp.model.changed': '模型：{model}',
  'sp.model.switch_failed': '无法切换模型：{error}',
  'sp.model.vision_inherit': '图片和屏幕截图也由该模型读取。',
  'sp.model.vision_dedicated': '图片和屏幕截图由 {model} 读取。',
  ...
  'st.skills.add_file': "从 .md 文件导入",
  'st.skills.file_hint': "也可以将 .md 文件拖放到此卡片上。",
  'st.skills.added_multi': "已添加 {count} 个技能。",
  'st.skills.error.file_read': "无法读取 {name}。",
  'st.skills.error.file_type': "{name} 不是受支持的技能文件（.md、.markdown、.txt）。",
  'st.skills.edit': "编辑",
  'st.skills.editing': "正在编辑：{name}",
  'st.skills.editing_hint': "保存修改会将此打包/URL 导入的技能转换为自定义文本，之后不再自动更新。",
  'st.skills.save_edit': "保存更改",
  'st.skills.cancel_edit': "取消编辑",
  'st.skills.updated': "技能已更新。",
  'st.skills.error.update_missing': "该技能已不存在——可能已在其他窗口中被删除。",
  'st.skills.chars_count': "{count} / {max} 个字符",
  'st.skills.chars_over': "超过 {max} 个字符的内容将在保存时被截断。",
  'st.skills.draft.title': "将保存为",
  'st.skills.draft.modes': "模式：{modes}",
  'st.skills.draft.intents': "意图：{intents}",
  ...
  'sp.attach.mime_mismatch': '{name} 已被拒绝：文件内容与其声明的类型不符。',
  'sp.attach.doc_legacy': '{name} 是旧版 Word 文件（.doc）。请另存为 .docx 后重新附加。',
  'sp.attach.drop_hint': '将文件拖放到此处附加',
  'sp.attach.hint.pdf': 'PDF · {pages} 页',
  'sp.attach.hint.pdf_scan': '扫描版 PDF · {pages} 页 — 需要具备视觉能力的模型',
  'sp.attach.hint.invalid_docx': '无法读取的 .docx 文件',
  'sp.attach.outcome.sent_native': '已发送给模型',
  'sp.attach.outcome.sent_text': '已以提取文本形式发送',
  'sp.attach.outcome.sent_pages': '已以页面渲染图像形式发送',
  'sp.attach.outcome.reference': '已作为引用附加，代理可按需读取',
  'sp.attach.outcome.needs_vision': '未发送 — 所选模型无法查看图像',
  'sp.attach.outcome.needs_vision_scan': '未发送 — 此扫描版 PDF 需要视觉模型',
  'sp.attach.outcome.policy': '未发送 — 被安全策略阻止',
  'sp.attach.outcome.expired': '未发送 — 本地保存的文件已过期，请重新附加',
  'sp.attach.outcome.error': '未发送 — 文件无法处理',
  "sp.queue.label": "已排队",
  ...
  "sp.plan.auto_approved_mode": "已按权限模式“{mode}”自动批准计划 — 正在运行…",
  ...
  'st.attach.heading': '附件',
  'st.attach.desc': '附加的文件保存在本设备上，因此输入框中的附件在重新加载后仍会保留，代理也能按需读取文件的更多内容。仅当消息发送给你所选的提供商时，文件内容才会离开设备。',
  'st.attach.retention.label': '保留附加文件',
  'st.attach.retention.desc': '附加文件在最后一次使用后于本地存储中保留的时长。',
  'st.attach.retention.ttl24h': '最长 24 小时',
  'st.attach.retention.session': '仅当前浏览器会话',
  'st.attach.usage': '已存储 {count} 个文件 · {size}',
  'st.attach.clear.btn': '删除所有附加文件',
  'st.attach.clear.done': '已删除所有附加文件。',
  'st.attach.clear.failed': '无法删除附加文件。',
};
