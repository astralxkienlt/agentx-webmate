...
browser.storage.local.get([ACT_WARNING_DISMISSED_KEY]).then((stored) => {
...
void loadPermissionMode(browser.storage.local).then((mode) => {
  permissionMode = mode;
...
async function init() {
  ...
  setMode(await loadAgentModePreference(browser.storage.local), { remember: false, instant: true });
...
async function parseSlashCommands(text, tabId = currentTabId, options = {}) {
  ...
  if (command.value === '/dangerously-skip-permissions') {
    ...
    await savePermissionMode(browser.storage.local, PermissionMode.BYPASS);
    applyPermissionModeToComposer();
  ...
  try {
    ...
    if (renderToCurrentTab && currentTabId === tabId && isTabAbortRequested(tabId)) {
      ...
      if (textEl && getStreamedAssistantText(textEl) === String(res.content)) {
        ...
        if (!renderSubscribeError(textEl, res.content, modeForSend)) {
...
function addErrorRetryButton(msgEl, retryPayload) {
  if (!msgEl || !retryPayload?.text
      || msgEl.querySelector('.error-retry-btn, .planner-request-failure-retry-btn')) return;
...
function rememberAgentMode(mode) {
  ...
  void saveAgentModePreference(browser.storage.local, mode);
}
 ...
 * the next sidebar document opens in it; pass `false` for modes the panel
 * forces on the user (selection-scoped conversations), which are constraints
 * rather than preferences and must not overwrite what they picked. `instant`
 * skips the pill's slide animation, for the restore that runs before the first
 * paint.
 */
...
async function ingestFiles(fileList, tabId = currentTabId) {
  const numericTabId = normalizeAttachmentTabId(tabId);
  if (numericTabId == null) return;
 ...
 */
async function restoreStoredAttachmentChips(tabId = currentTabId) {
  const numericTabId = normalizeAttachmentTabId(tabId);
  const store = attachmentStoreOrNull();
  ...
  try {
    ...
    if (!sameTabId(currentTabId, numericTabId)) return;
    const pending = getPendingAttachmentsForTab(numericTabId);
...
if (attachBtn && fileAttachInput) {
  ...
  fileAttachInput.addEventListener('change', () => {
    ...
    ingestFiles(fileAttachInput.files, currentTabId);
...
inputEl.addEventListener('paste', (event) => {
  ...
  if (!files.length) return;
  event.preventDefault();
  void ingestFiles(files, currentTabId);
});
