...
> Version 0.1.15 · Manifest V2 · Background Page
...
1. **A mode only answers unanswered questions.** `PermissionManager.check()`
   ...
   and `page_actions` from answering for a callback the PAGE wrote. Only
   `bypass` outranks it, because that mode already runs `execute_js` unprompted
   on the same page. The case that needs the flag, a WebMCP page callback,
   exists only in the Chrome build; the flag is passed here too so the two
   trees decide alike.
