...
export class ProviderManager {
  ...
  _getPlatformInfo() {
    return new Promise((resolve) => {
      // A callback the browser never invokes would park load() — and with it
      // every background message — forever. Settle with nothing instead.
      const timeoutId = setTimeout(() => resolve(null), 2_000);
      const settle = (info) => {
        clearTimeout(timeoutId);
        resolve(info || null);
      };
      try {
        const maybePromise = chrome.runtime.getPlatformInfo((info) => settle(info));
        if (maybePromise && typeof maybePromise.then === 'function') {
          maybePromise.then((info) => settle(info), () => settle(null));
        }
      } catch {
        settle(null);
      }
