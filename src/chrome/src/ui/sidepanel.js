...
(async function initOnboarding() {
  ...
  const stored = await chrome.storage.local.get('onboardingComplete').catch(() => ({ onboardingComplete: true }));
...
async function init() {
  ...
  // A panel document restored against a window that no longer exists (Edge
  // suspends and revives panels) rejects this query; the panel must still
  // finish booting so the tab/window listeners below can re-bind it.
  ...
  // Load settings that affect the composer state. Defaults on a failed read:
  // a rejection here used to abort the rest of init() and leave a silent,
  // inert panel.
  const stored = await chrome.storage.local.get(['verboseMode', 'alwaysAllowApiMutations']).catch(() => ({}));
