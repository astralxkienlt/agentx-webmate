...
This package is self-contained: `scripts/agentx-webmate-mcp.mjs` is the MCP
...
2. **Install this skill** into the active profile so the path is
   `$AGENTX_HOME/skills/autonomous-ai-agents/webmate/SKILL.md` (unzip the
...
3. **Register the bundled server** (idempotent; re-run after moving the skill):
   ...
   ```
   It writes `mcp_servers.webmate` → `node …/scripts/agentx-webmate-mcp.mjs`
   ...
   `webmate` catalog entry, `agentx mcp install official/webmate` is an
   equivalent alternative.
4. **Extension attached.** In a Chromium browser with AgentX WebMate installed:
...
6. **WebMate has its own model.** WebMate runs its own LLM loop (AgentX Cloud
   after signing in, or any provider configured in its Settings). AgentX's model
   choice does not apply to the browser side.
7. Only **one bridge at a time**: 17373 (WebMate Cloud), 17374 (this MCP
...
```bash
python3 "${AGENTX_HOME:-$HOME/.agentx}/skills/autonomous-ai-agents/webmate/scripts/check_bridge.py"
```
