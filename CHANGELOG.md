...
- **The skill form previews what a draft will be saved as.** A live panel under the textarea shows the name, summary, modes, intents, and declared tools the current text parses to — a malformed `webbrain-skill` fence now shows up as Act-only defaults *before* saving instead of after — plus a character counter that flags text beyond the 20,000-character cap, which used to be truncated silently. The edit/demotion rules live in `agent/skills.js` as pure helpers (`applySkillEdit`, `describeSkillDraft`, file-name helpers) covered by unit tests in both browser builds, and the redesigned form was exercised end-to-end in a real browser against a storage stub.
