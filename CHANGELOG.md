...
- **The AgentX Skill Hub now lives at `https://skills.astralx.com.vn`.** The default hub address (`services.skillHubBaseUrl` in `brand/brand.config.json`) points there, and so does the only origin allowed to message the extension (`externally_connectable`, Chrome/Edge), which the brand build derives from the same value. A hub address saved by hand in Settings → Skills still wins over the default, so a browser that was pointed at the old host keeps that override until it is cleared.
