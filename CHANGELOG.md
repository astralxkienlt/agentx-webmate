...
### Changed
- The version strings jump from 0.1.16 straight to 0.1.19: 0.1.17 and 0.1.18 were bumped but never cut as releases, so this entry carries everything landed since 0.1.16.
- **Enter now sends the selected-text question; Alt+Enter breaks the line.** The floating ask bubble's textarea used to treat Enter as a newline and only sent on Ctrl/Cmd+Enter — an unlabelled chord nobody guesses, so drafts sat unsent. Enter alone now submits, Alt+Enter inserts a hard newline at the caret (done by hand via `setRangeText` because not every platform maps Alt+Enter to an insertion, with the send button's disabled state refreshed through a real `input` event), Shift+Enter keeps its native newline, and the old Ctrl/Cmd+Enter still sends for muscle memory. Two Enters that must never send, don't: the trusted Enter that confirms an IME composition — how Vietnamese Telex and CJK input commit text, detected via `isComposing`/`keyCode 229` — and any synthetic Enter a page script dispatches at the surface, which is untrusted and now ignored outright (clicks were already `isTrusted`-gated; the keyboard submit path wasn't). An Enter on an empty box neither sends nor adds a stray newline. Browser fixtures drive all five paths — multiline compose, IME keyCode via CDP, synthetic dispatch, empty box, and the legacy chord — for both browser builds, and the suite pins the contract plus chrome↔firefox byte parity of the content script.
- **The selected-text ask bubble now wears the netMind brand (branded builds).** It was the third content script still painting upstream's indigo from inside its closed shadow DOM — the "?" button, lavender hovers, violet-navy dark palette — next to a red product. The brand layer now recolours it end to end: raspberry `#f1143c` accents with `#d41034` hover on the light theme, the lightened `#ff4d6d`/`#ff6a84` pair with warm dark neutrals from the netMind Chat palette in dark mode, red selection highlight and focus rings, blush row hovers. The question-mark glyph gives way to an inline-SVG recreation of the ribbon N-mark (traced against `icon128.png`, ~3% silhouette deviation, CSP-immune because nothing loads as a resource — the extension's icons stay non-web-accessible), and the popup opens with a small N-mark plus "netMind" wordmark row. Colour swaps ride `brand.config.json` rules so they survive upstream rearranging the stylesheet; only the structural delta (logo markup, brand row, and the dark-theme accent overrides upstream never declares) lives in patch `090-selection-shortcut-netmind.patch`, which fails the build loudly on drift. `src/` stays pristine upstream — the suite still asserts the unbranded "?" there — while the brand test asserts the branded output carries the N-mark twice, zero indigo leftovers in either theme, byte-identical chrome/firefox output, and the rebranded Vietnamese strings.
...
- **The estimated-cost fallback for metered usage works again.** A merge had mangled `_usageTokenCounts` into a second `_chat(provider, …)` declaration — dead code, shadowed by the real `_chat` below it — so a metered response arriving without an explicit cost field made the cost recorder throw "not a function" instead of estimating the spend from token counts. Chrome tree only; Firefox never carried the duplicate.
...
- **Bypass permissions now means bypass.** The widest rung still stopped a run twice. The planner's review card was never wired to the mode at all, so a task could block on an approval before its first tool call even with every prompt turned off; and a WebMCP page callback was hard-wired to ask in every mode in the Chrome build, while Firefox already let Bypass through, so the two trees had drifted apart. Plan review is now auto-approved under Bypass — overriding even a plan-review setting of *always* — and the run says so, naming the mode that approved it instead of a confidence score that had no say in it. A `clarify()` question from the model and the workflow target-healing card are deliberately still asked in every mode, Bypass included: neither is a permission.
- A mandatory gate still survives the *narrower* permissive modes. WebMCP invocations run arbitrary page logic behind a documented two-gate boundary; the `requireExplicitGrant` flag opts them out of the mode policy, so they still raise a card in Auto and Accept page actions. Bypass is the single exception, since that rung already runs page scripts unprompted on the same page — more authority than invoking a tool the page chose to expose. The threshold is recomputed from the live mode, so widening to Bypass while a WebMCP card is open releases that card too.
- **A waited `clarify` timeout no longer stalls a Bypass run.** When a `clarify` question times out with nobody answering, the auto-selected answer is not treated as your authorization: the next consequential action and any success completion stay blocked until you reply for real. That is a run halted to collect an approval, so Bypass — the one rung whose name promises otherwise — now releases it. The question itself is still asked in every mode; only what happens when nobody replies changes. The decision is read live and never recorded, so the guard stays armed and dropping back to a stricter mode restores the block for whatever comes next, with the blocked-attempt counter untouched. Auto and Accept page actions deliberately keep the block: they pre-approve interaction you can watch happen, not the substitution of silence for an answer.
- The system prompt now points the model at the clarify result's own `authorized` field instead of ranking answers by where they came from. Six prompt strings used to state flatly that a timed-out answer is never approval, which contradicted the runtime the moment a Bypass run timed out — the model could refuse an action the mode had already allowed.
...
- Permission-mode copy now matches behaviour in all 23 locales. Auto's description lists running page scripts among what it still asks about (it always did), and Bypass's says it accepts every permission, runs the plan without asking you to approve it, and keeps going with the default answer when a question times out unanswered.
